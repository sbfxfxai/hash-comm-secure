...
type SortBy = 'name' | 'date' | 'reputation' | 'interactions';
...
export const ContactManager: React.FC = memo(() => {
    ...
    const analytics = useMemo(() => {
        ...
        return {
            ...
            totalSatsReceived,
            totalInteractions,
            averageReputation: total > 0 ? contacts.reduce((sum, c) => sum + c.reputation, 0) / total : 0
        };
    }, [contacts]);
    ...
    const handleExportContacts = useCallback((contactIds?: string[]) => {
    ...
    const handleImportContacts = useCallback((csvContent: string) => {
        try {
                            ...
                            <ScrollArea className="h-[calc(100vh-280px)]">
                                <div className="p-4 space-y-2">
                                    {filteredContacts.map(contact => (
                                        <Card key={contact.id} className={`cursor-pointer transition-all ${selectedContacts.includes(contact.id) ? 'ring-2 ring-primary' : ''
                                            }`}>
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            {contact.lightningAddress && <Zap className="h-4 w-4 text-orange-500" />}
                                                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Avg: {analytics.total > 0 ? (analytics.totalInteractions / analytics.total).toFixed(1) : '0'} per contact
                                </p>
                            </CardContent>
                        </Card>
            ...
            <Dialog open={showImportDialog} onOpenChange={setShowImportDialog}>
                <DialogContent>
                                ...
                                onChange={(e) => {
                                    ...
                                    if (file) {
                                        ...
                                        reader.onload = (event) => {
                                            ...
                                            const csvDataElement = document.getElementById('csvData') as HTMLTextAreaElement;
                                            if (csvDataElement) {
                                                csvDataElement.value = content;
                                            }
                                        };
                    ...
                    <DialogFooter>
                        ...
                        <Button onClick={() => {
                            const csvDataElement = document.getElementById('csvData') as HTMLTextAreaElement;
                            const csvData = csvDataElement?.value || '';
